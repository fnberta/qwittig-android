 ...
 */
public class AddPurchaseSaveWorker extends BaseWorker<Purchase, AddPurchaseSaveWorkerListener> {

    static final String WORKER_TAG = AddPurchaseSaveWorker.class.getCanonicalName();
    @Inject
    PurchaseRepository mPurchaseRepo;
    Purchase mPurchase;
    byte[] mReceiptImage;

    public AddPurchaseSaveWorker() {
        // empty default constructor
    }

    /**
     * Constructs a new {@link AddPurchaseSaveWorker} with a {@link Purchase} object and optionally
    ...
    AddPurchaseSaveWorker(@NonNull Purchase purchase, @Nullable byte[] receiptImage) {
        mPurchase = purchase;
        mReceiptImage = receiptImage;
    }

    /**
     * Attaches a new instance of a {@link AddPurchaseSaveWorker}.
     *
     * @param fm           the fragment manager to use for the transaction
     * @param purchase     the {@link Purchase} object to save
     * @param receiptImage the receipt image to attach to the purchase
     * @return a new instance of a {@link AddPurchaseSaveWorker}
     */
    public static AddPurchaseSaveWorker attach(@NonNull FragmentManager fm,
                                               @NonNull Purchase purchase,
                                               @Nullable byte[] receiptImage) {
        AddPurchaseSaveWorker worker = (AddPurchaseSaveWorker) fm.findFragmentByTag(WORKER_TAG);
        if (worker == null) {
            worker = new AddPurchaseSaveWorker(purchase, receiptImage);
