...
public abstract class StatsBaseFragment extends BaseFragment {
    ...
    final void calcStatsWithWorker(@StatsCalcWorker.StatsType int statsType,
                                   @NonNull String year, int month) {
        final FragmentManager fragmentManager = getFragmentManager();
        Fragment statsWorker = fragmentManager.findFragmentByTag(getWorkerTag());

        // If the Fragment is non-null, then it is currently being
        // retained across a configuration change.
        if (statsWorker == null) {
            statsWorker = StatsCalcWorker.newInstance(statsType, year, month);
